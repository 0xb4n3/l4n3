...
plugins=(git web-search sudo copypath copyfile zsh-autosuggestions zsh-syntax-highlighting)
...
alias ipa='ip -o -4 addr show up | awk "{print \$2, \$4}"'


# prompt de base
#PROMPT='${debian_chroot:+(%{$debian_chroot%}%F{green}%n@%m%f:%F{blue}%~%f$ '

# custom afowler theme
venv_prompt() {
  [[ -n "$VIRTUAL_ENV" ]] && echo "%F{red}($(basename "$VIRTUAL_ENV"))%f "
}

PROMPT='$(venv_prompt)%B%F{blue}%n%f %F{white}%~%f $(hg_prompt_info)$(git_prompt_info)%B%(!.%F{red}.%F{blue})»%f%b '
RPS1='%(?..%F{red}%? ↵%f)'
...
# fzf keybindings for zsh (Ctrl-R, Ctrl-T, Alt-C)
